        ...
        protected void BtnSrink_Click(object sender, EventArgs e)
        {
            ...
            foreach (IListBlobItem item in container.ListBlobs(null, false))
            {
                if (item.GetType() == typeof(CloudBlockBlob))
                {
                    ...
                    using (var fileStream = System.IO.File.OpenWrite(@"D:\image\crimecop\crimecopcpp\crimecopcpp\images\" + blobitem.Name +".jpg"))
                    {
                        blob.DownloadToStream(fileStream);
                    }

                    FileInfo fi = new FileInfo(@"D:\image\crimecop\crimecopcpp\crimecopcpp\images\"+blobitem.Name + ".jpg");
